...
void main() {
  group('Phase 04 Nyquist Validation:', () {
    group('1. Wikidata API Client & Policy Compliance:', () {
      test('wikidata_api.dart should exist and have rate limiting, User-Agent, and retry policies configured', () {
        final file = File('lib/features/wikidata/data/datasources/wikidata_api.dart');
        ...
        expect(content, contains('SonicNomad/1.0.0 ( contact@sonicnomad.app )'));
    ...
    group('2. Clean Architecture Layer Separation:', () {
      test('Domain layers should not import Data layers (boundary enforcement)', () {
        ...
        for (final file in files) {
          ...
          expect(content, isNot(contains('import \'../../data')),
              reason: '${file.path} violates clean architecture by importing data layer elements.');
          expect(content, isNot(contains('import \'../data')),
              reason: '${file.path} violates clean architecture by importing data layer elements.');
          expect(content, isNot(contains('package:http/http.dart')),
              reason: '${file.path} violates clean architecture by importing direct HTTP package.');
        }
      });

      test('WikidataRepository should be abstract/interface in domain', () {
        final file = File('lib/features/wikidata/domain/repositories/wikidata_repository.dart');
        expect(file.existsSync(), isTrue);
      ...
      test('WikidataRepositoryImpl should exist in data layer and implement domain interface', () {
        final file = File('lib/features/wikidata/data/repositories/wikidata_repository_impl.dart');
        expect(file.existsSync(), isTrue);
    ...
    group('3. Data Models vs Domain Entities Boundary:', () {
      test('Genre entity should not contain serialization logic', () {
        final genreEntityFile = File('lib/features/wikidata/domain/models/genre.dart');
    ...
    group('4. Canvas Layout & Specialized Widgets Metric Enforcement:', () {
      test('GenreNodeWidget should have standard 120x120px circular size metric', () {
        final file = File('lib/features/canvas/presentation/widgets/genre_node_widget.dart');
        expect(file.existsSync(), isTrue);
      ...
      test('GraphLayoutEngine should centralize node and radial geometry dimensions', () {
        ...
        expect(GraphLayoutEngine.getNodeSize('genre'), equals(const Size(120.0, 120.0)));
        expect(GraphLayoutEngine.getNodeSize('artist'), equals(const Size(180.0, 90.0)));
        expect(GraphLayoutEngine.getNodeSize(null), equals(const Size(180.0, 90.0)));
      });
    });
  });
}
