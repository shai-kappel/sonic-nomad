...
void main() {
  group('Phase 05 Nyquist Validation:', () {
    group('1. Firebase Initialization & Platform Wiring:', () {
      test(
        'lib/main.dart should initialize Firebase before running dependencies and UI',
        () {
          ...
          expect(
            content.indexOf('Firebase.initializeApp'),
      ...
      test(
        'firebase_options.dart should exist and contain DefaultFirebaseOptions',
        () {
    ...
    group('2. Clean Architecture Layer Separation (Auth):', () {
      test(
        'Auth Domain layers should not import Data layers (boundary enforcement)',
        () {
      ...
      test(
        'AuthRepositoryImpl should exist in data layer and implement domain interface',
        () {
          final file = File(
    ...
    group('3. UI Affordances & Guest-Safe Boundaries:', () {
      test(
        'CanvasPage should contain AuthStatusButton as a non-blocking overlay',
        () {
          final file = File(
      ...
      test(
        'No guest-mode local persistence package or custom local-cache write path should be used in Auth feature',
        () {
    ...
    group('4. Required Phase 5 Automated Test Suites:', () {
      test(
        'Auth BLoC unit tests and AuthEntrySheet widget tests must exist',
        () {
          final blocTest = File(
            'test/features/auth/presentation/bloc/auth_bloc_test.dart',
          );
          final widgetTest = File(
            'test/features/auth/presentation/widgets/auth_entry_sheet_test.dart',
          );

          expect(
            blocTest.existsSync(),
            isTrue,
            reason: 'AuthBloc unit test suite is missing.',
          );
          expect(
            widgetTest.existsSync(),
            isTrue,
            reason: 'AuthEntrySheet widget test suite is missing.',
          );
        },
      );
    });
